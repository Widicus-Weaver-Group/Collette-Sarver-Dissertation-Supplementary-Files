...
cube_file = 'IRAS-4B-hr-10_line.fits' # 
...
# ***********************************************************************************************************************
subcube = cube[1679:1827,:,:]     # 1536:1641 shape(n_s,n_y,n_x) s is spectral axis (channels)
# 
# ***********************************************************************************************************************
...
try:
    os.remove('../../Moment-0-Maps-mult-emission/CH3OH-6-26/moment_0-CH3OD-7.fits')
except OSError:
    pass
...
moment_0.write('../../Moment-0-Maps-mult-emission/CH3OH-compact/moment_0-CH3OD-7.fits') 
with fits.open('../../Moment-0-Maps-mult-emission/CH3OH-compact/moment_0-CH3OD-7.fits') as hdum:
    #cubem = sc.read(hdum)
    ...
    vmin=round(3*rms[10],5) #This is 3*sigma_emission of your #-hr_line.fits cube
    #wcs = WCS(hdum[0]) 
...
ell = my_beam.ellipse_to_plot(107,87, pixscale) 
#Normal: 127,107
#CH3OH-maser:
ell.set_facecolor('w')
_ = ax.add_artist(ell)
...
ax.text(0.58, 0.96, textstr, transform=ax.transAxes, fontsize=12,
...
ax.text(0.04, 0.96, f'$\\sigma$={vmax}', transform=ax.transAxes, fontsize=12,
        verticalalignment='top', bbox=props, color='white')  #extended: 0.04, 0.96
